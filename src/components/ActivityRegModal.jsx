...
export default function ActivityRegModal({ activity, event, onClose }) {
  ...
  const handleDownloadTicket = async () => {
    ...
    const checkinUrl = `${window.location.origin}/checkin?rid=${regId}`
        ...
        {isSoldOut ? (
          ...
          <div className="arm-success">
            <div className="arm-success__check">
              <svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="#5a6c1e" strokeWidth="2">
                <circle cx="12" cy="12" r="10" stroke="#5a6c1e" fill="#f0fdf4"/>
                <polyline points="7 12 11 15 17 8" strokeWidth="2.5"/>
              </svg>
            </div>
            ...
            <p className="arm-success__msg">
              Presenta este ticket al llegar. Puedes descargarlo o recibirlo por WhatsApp.
            </p>
            ...
            <div className="arm-success__actions">
              ...
              <button className="arm-success__btn arm-success__btn--wa" onClick={handleShareWhatsApp}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.132.562 4.13 1.541 5.858L.057 23.486a.5.5 0 0 0 .614.614l5.628-1.484A11.945 11.945 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22a9.944 9.944 0 0 1-5.031-1.362l-.36-.214-3.736.985.985-3.736-.214-.36A9.944 9.944 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
                <span>Enviar por WhatsApp</span>
              </button>
            </div>

            <button className="arm-success__close-btn" onClick={onClose}>
              Cerrar
            </button>
          </div>
        ) : step === 1 ? (
